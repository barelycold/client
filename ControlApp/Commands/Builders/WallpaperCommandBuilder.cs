...
public class WallpaperCommandBuilder() : FileCommandBuilder("Wallpaper Command", "Wallpaper File") {
    public override CommandStructure BuildCommand(Panel inputPanel) {
        ...
        else // implies URL input
        {
            ...
            else if (!Utilities.IsImageFile(url)) { // I know the "else" here is redundant, but it emphasizes that these two clauses are mutually exclusive
